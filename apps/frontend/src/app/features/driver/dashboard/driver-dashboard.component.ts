...
@Component({
  ...
  template: `
    <div class="h-screen flex flex-col bg-slate-900">
      <!-- Header -->
      <header class="bg-slate-800/80 backdrop-blur-xl border-b border-slate-700 px-4 py-3 flex items-center justify-between z-10">
      ...
      @if (driver()?.approvalStatus === 'PENDING') {
        <div class="bg-amber-500/10 border-b border-amber-500/20 px-4 py-3">
          <div class="flex items-center gap-3">
            ...
            <p class="text-amber-300 text-sm">Your account is pending approval. You cannot go online yet.</p>
          </div>
        </div>
      }
      ...
      <div class="flex-1 relative">
        ...
        @if (state() === 'ride_offer' && currentOffer()) {
          <div class="absolute inset-0 bg-slate-900/80 backdrop-blur-sm flex items-end justify-center p-4">
            <div class="w-full max-w-md bg-slate-800 rounded-2xl border border-slate-700 overflow-hidden animate-slide-up">
              <div class="p-6">
                ...
                <div class="space-y-3 mb-6">
                  <div class="flex items-center gap-3 p-3 bg-slate-900/50 rounded-xl">
                    ...
                    <div>
                      ...
                      <p class="text-white">{{ currentOffer()!.pickupLat.toFixed(4) }}, {{ currentOffer()!.pickupLon.toFixed(4) }}</p>
                    </div>
                  </div>
                  <div class="flex items-center gap-3 p-3 bg-slate-900/50 rounded-xl">
                    ...
                    <div>
                      ...
                      <p class="text-white">{{ currentOffer()!.dropoffLat.toFixed(4) }}, {{ currentOffer()!.dropoffLon.toFixed(4) }}</p>
                    </div>
                  </div>
                </div>
                ...
                <div class="flex items-center justify-between mb-6 text-center">
                  <div>
                    <p class="text-2xl font-bold text-white">{{ (currentOffer()!.estimatedDistance / 1000).toFixed(1) }}</p>
                    <p class="text-xs text-slate-400">km</p>
                  </div>
                  <div class="w-px h-8 bg-slate-700"></div>
                  <div>
                    <p class="text-2xl font-bold text-white">{{ Math.round(currentOffer()!.estimatedDuration / 60) }}</p>
      ...
      <div class="bg-slate-800 border-t border-slate-700 p-4">
        @switch (state()) {
          ...
          @case ('in_trip') {
            <div class="space-y-3">
              <div class="p-4 bg-green-500/10 border border-green-500/20 rounded-xl flex items-center justify-between">
                <div>
...
export class DriverDashboardComponent implements OnInit, OnDestroy {
  ...
  private async sendLocationUpdate(): Promise<void> {
    try {
      ...
      this.wsService.updateLocation(-1.9403, 29.8739);
    }
  }
  ...
  acceptOffer(): void {
    ...
    this.wsService.acceptRide(offer.rideRequestId);
    this.stopOfferTimeout();
  ...
  rejectOffer(): void {
    const offer = this.currentOffer();
    if (offer) {
      this.wsService.rejectRide(offer.rideRequestId);
    }
    this.stopOfferTimeout();
    this.currentOffer.set(null);
